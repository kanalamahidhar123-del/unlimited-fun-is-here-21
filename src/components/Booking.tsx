...
export default function Booking() {
  ...
  const nextAvailableDates = (form.visit_date && (isSelectedDateFull || !isSelectedDateOpen))
    ? getNextAvailableOpenDates(form.visit_date, 4)
    : [];
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    {!isSelectedDateOpen && form.visit_date && (
                      <div className="mt-2.5 rounded-xl bg-flame-500/15 border border-flame-500/40 p-3 text-xs text-flame-300 space-y-2">
                        <div className="flex items-center gap-2 font-bold text-flame-400">
                          <AlertCircle className="h-4 w-4 flex-shrink-0" />
                          <span>⛔ This date is currently closed by administration.</span>
                        </div>
                        {nextAvailableDates.length > 0 && (
                          <div className="pt-2 border-t border-flame-500/20 space-y-1.5">
                            <span className="text-[11px] text-gray-300 block font-semibold">Available Open Dates:</span>
                            <div className="flex flex-wrap gap-1.5">
                              {nextAvailableDates.map((nd) => (
                                <button
                                  key={nd.date}
                                  type="button"
                                  onClick={() => update('visit_date', nd.date)}
                                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-volt-500 text-ink-950 font-bold text-[11px] hover:bg-volt-400 transition-colors shadow-sm"
                                >
                                  {formatFriendlyDate(nd.date)} ({nd.remaining} left)
                                </button>
                              ))}
                            </div>
                          </div>
                    ...
                    {isSelectedDateOpen && isSelectedDateFull && (
                      <div className="mt-2.5 rounded-xl bg-flame-500/15 border border-flame-500/40 p-3 text-xs text-flame-300 space-y-2">
                        <div className="flex items-center gap-2 font-bold text-flame-400">
                          <AlertCircle className="h-4 w-4 flex-shrink-0" />
                        ...
                        {nextAvailableDates.length > 0 && (
                          <div className="pt-2 border-t border-flame-500/20 space-y-1.5">
                            <span className="text-[11px] text-gray-300 block font-semibold">Next Available Dates:</span>
                            <div className="flex flex-wrap gap-1.5">
                              {nextAvailableDates.map((nd) => (
                                <button
                                  key={nd.date}
                    ...
                    {form.visit_date && isSelectedDateOpen && !isSelectedDateFull && (
                      <div className="mt-2 flex items-center justify-between px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                        <span className="text-[11px] text-emerald-400 font-bold flex items-center gap-1.5">
                          <CheckCircle2 className="h-3.5 w-3.5" />
                          Available — {selectedDateBooked} / {DAILY_CAPACITY} bookings
                        </span>
